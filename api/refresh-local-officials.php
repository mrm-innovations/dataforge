...
if (!defined('LOCAL_OFFICIALS_SHEET_ID') || !LOCAL_OFFICIALS_SHEET_ID) {
  http_response_code(500);
  echo json_encode(['ok' => false, 'error' => 'LOCAL_OFFICIALS_SHEET_ID not set.']);
  exit;
}
$sheetId = LOCAL_OFFICIALS_SHEET_ID;
$gid = defined('LOCAL_OFFICIALS_SHEET_GID') ? LOCAL_OFFICIALS_SHEET_GID : '0';
$python = defined('PYTHON_BIN') ? PYTHON_BIN : 'python';
$script = realpath(__DIR__ . '/../scripts/fetch_local_officials.py');
if ($script === false) {
  http_response_code(500);
  echo json_encode(['ok' => false, 'error' => 'fetch_local_officials script not found.']);
  exit;
}
$cmd = escapeshellarg($python) . ' ' . escapeshellarg($script) . ' --sheet-id ' . escapeshellarg($sheetId) . ' --gid ' . escapeshellarg($gid);
$descriptor = [
  0 => ['pipe', 'r'],
  1 => ['pipe', 'w'],
  2 => ['pipe', 'w'],
