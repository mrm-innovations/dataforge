...
if (!defined('DEMOGRAPHY_SHEET_ID') || !DEMOGRAPHY_SHEET_ID) {
  http_response_code(500);
  echo json_encode(['ok' => false, 'error' => 'DEMOGRAPHY_SHEET_ID not set.']);
  exit;
}
$sheetId = DEMOGRAPHY_SHEET_ID;
$gid = defined('DEMOGRAPHY_SHEET_GID') ? DEMOGRAPHY_SHEET_GID : '0';
$python = defined('PYTHON_BIN') ? PYTHON_BIN : 'python';
$script = realpath(__DIR__ . '/../scripts/fetch_demography.py');
if ($script === false) {
  http_response_code(500);
  echo json_encode(['ok' => false, 'error' => 'fetch_demography script not found.']);
  exit;
}
