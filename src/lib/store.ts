...
async function loadDemographyOverlay(base: string) {
  const dir = `${location.pathname.replace(/\/[^/]*$/, '/') || '/'}`
  const candidates = [`${base}demography.json`, `${dir}demography.json`, `/demography.json`]
  ...
  data.forEach((entry) => {
    const psgc = (entry.psgc || '').trim()
    const key = psgc || `${normalizeName(entry.province)}|${normalizeName(entry.lgu)}`
    if (key) map.set(key, entry)
  })
  store.LGUS = (store.LGUS || []).map((g) => {
    const psgc = (g.psgc || '').trim()
    const key = psgc || `${normalizeName(g.province)}|${normalizeName(g.lgu)}`
    const match = key ? map.get(key) : null
    if (!match) return g
    return {
      ...g,
      psgc: match.psgc || g.psgc,
      province: match.province || g.province,
      lgu: match.lgu || g.lgu,
      type: match.type || g.type,
      income_class: match.income_class || g.income_class,
      population: match.population != null ? match.population : g.population,
    }
  })
}
...
export async function reloadDemography() {
  await loadDemographyOverlay(assetsBase)
  recomputeTotals()
...
export function bandLabelFor(auditKey: string, value: number | null | undefined): string {
  ...
  if (!key) return '-'
  return bandLabelForKey(auditKey, key)
}

export function bandLabelForKey(auditKey: string, bandKey: BandKey | string | null | undefined): string {
  if (!bandKey) return '-'
  const key = bandKey as BandKey
  const meta = (store.AUDITS || {})[auditKey] || {}
  const labels = (meta as any).labels || {}
  const labelsShort = (meta as any).labels_short || {}
  const arr = bandsArrayFor(auditKey)
  const fromBandArray = arr?.find((b) => b.key === key)
  if (fromBandArray?.label) return stripParen(String(fromBandArray.label))
