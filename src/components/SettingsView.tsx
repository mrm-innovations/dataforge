...
import { store, filterRows, setAudit } from '@/lib/store'
...
const sheetLink = (id: string, gid = '0') => `https://docs.google.com/spreadsheets/d/${id}/edit#gid=${gid}`

const SHEET_SOURCES: Array<SheetSource & { url: string }> = [
  {
    key: 'field-officers',
    title: 'Field Officers Directory',
    description: 'Roster of C/MLGOOs, provinces/HUCs, assignments, and contact info.',
    sheetId: '1J8QksAsygyTmamYNV9Bsc3TusUgVvRKWexYQTHVzMQg',
    gid: '0',
    usedIn: 'Field Officers tab',
    refreshHint: 'Click "Refresh Directory" (admin action) after edits.',
  },
  {
    key: 'local-officials',
    title: 'Local Officials (2025-2028)',
    description: 'Governor, SP/SP, city/municipal officials used by the Local Officials dashboard.',
    sheetId: '1gkIHGIp70gMSpX-1NYNc2wmoPTw46R7EOrdmPzLVYs8',
    gid: '0',
    usedIn: 'Local Officials tab',
    refreshHint: 'Use "Refresh Local Officials" (admin action).',
  },
  {
    key: 'demography',
    title: 'Demography Snapshot',
    description: 'Population, income class, component city data piped into Demography widgets.',
    sheetId: '1AuXsBrc7r_s9bfhg7JPrq1MiJKO8jmDYgIeXR-MiYcc',
    gid: '0',
    usedIn: 'Demography tab',
    refreshHint: 'Use "Refresh Demography" inside the Demography tab.',
  },
  {
    key: 'adac',
    title: 'ADAC Scores',
    description: 'Raw ADAC assessment per LGU (2019-2023).',
    sheetId: '1AIagKHZ0YtvW4i-TZJGYq_xl7b6oeb5aVlz4a-OJI00',
    gid: '0',
    usedIn: 'Dashboard > ADAC audit',
    refreshHint: 'Settings > Metrics & Imports > Refresh Audits.',
  },
  {
    key: 'sglg',
    title: 'SGLG',
    description: 'SGLG pass/fail + remarks powering donut + tables.',
    sheetId: '13e-AtBFytuQ9eXg_UoD8u6mAPyPIsrFO16pgutu-0dg',
    gid: '0',
    usedIn: 'Dashboard > SGLG audit',
    refreshHint: 'Settings > Metrics & Imports > Refresh Audits.',
  },
  {
    key: 'poc',
    title: 'POC',
    description: 'Peace and Order Council functionality ratings.',
    sheetId: '1yMv3IcJXOt6HGj0l8R6cjKPtqjhkP_zFRgZSwykCw1A',
    gid: '0',
    usedIn: 'Dashboard > POC audit',
    refreshHint: 'Settings > Metrics & Imports > Refresh Audits.',
  },
  {
    key: 'lcpc',
    title: 'LCPC',
    description: 'Local Council for the Protection of Children assessments.',
    sheetId: '1r-wuL3wo46aDA53I4IurVLCAvwQdNjgpjTCazSaKJBQ',
    gid: '0',
...
export function SettingsView({ authed = false }: { authed?: boolean }){
  // Admin auth gate
  const [adminAuthed, setAdminAuthed] = useState<boolean>(authed)
  ...
  const [activePanel, setActivePanel] = useState<'data' | 'metrics' | 'backup'>('data')
  const [uploadCanonLog, setUploadCanonLog] = useState<string>('')
  const uploadCanonRef = useRef<HTMLInputElement | null>(null)
  const [uploadingCanon, setUploadingCanon] = useState<boolean>(false)
  ...
  async function copySheetId(id: string){
    try {
      if (typeof navigator === 'undefined' || !navigator.clipboard?.writeText) throw new Error('Clipboard unavailable')
      await navigator.clipboard.writeText(id)
      addLog(`Copied sheet ID (${id}) to clipboard.`)
    } catch (err: any) {
      addLog('Copy sheet ID failed: ' + (err?.message || String(err)))
      if (typeof window !== 'undefined') window.prompt('Sheet ID', id)
    }
  }
  ...
  function downloadAuditConfigFile(){
  ...
  async function replaceCanonFromFile(file: File){
    try {
      setUploadingCanon(true)
      setUploadCanonLog(`Reading ${file.name}...`)
      const text = await file.text()
      const parsed = JSON.parse(text)
      if (!parsed || typeof parsed !== 'object') throw new Error('JSON root must be an object')
      if (!Array.isArray(parsed.lgus)) throw new Error('Missing "lgus" array')
      if (!parsed.meta) throw new Error('Missing "meta" object')
      store.CANON = parsed as any
      store.LGUS = parsed.lgus || []
      store.AUDITS = parsed.meta?.audits || {}
      setUploadCanonLog(`Loaded ${store.LGUS.length} LGUs. Uploading to server...`)
      try {
        setAudit(store.state.audit)
      } catch {}
      const resp = await fetch(`${apiBase()}api/save-json.php?file=lg-audits.json`, {
        method: 'POST',
        credentials: 'include',
      ...
      if (!resp.ok) {
        ...
        throw new Error(`HTTP ${resp.status}: ${msg || 'Failed to write lg-audits.json'}`)
      }
      setUploadCanonLog('Uploaded and replaced lg-audits.json on the server. Remember to redeploy/publish as needed.')
    } catch (e: any) {
      setUploadCanonLog(`Upload failed: ${e?.message || String(e)}`)
    } finally {
      setUploadingCanon(false)
      if (uploadCanonRef.current) uploadCanonRef.current.value = ''
    }
  }
    ...
    <div className="p-4">
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="inline-flex rounded-md border">
          ...
          <button
            className={`px-3 py-1 text-xs font-medium border-l ${activePanel === 'metrics' ? 'bg-primary text-white' : 'bg-white text-muted-foreground'}`}
            onClick={() => setActivePanel('metrics')}
          >
            Metrics & Imports
          </button>
          <button
            className={`px-3 py-1 text-xs font-medium rounded-r-md border-l ${activePanel === 'backup' ? 'bg-primary text-white' : 'bg-white text-muted-foreground'}`}
      ...
      {activePanel === 'data' && (
        <div className="mb-4 rounded-xl border bg-white p-4 space-y-4">
          <div>
            <h3 className="font-medium">Google Sheet Data Sources</h3>
            <p className="text-xs text-muted-foreground">
              All dashboards now pull from canonical Google Sheets. Manage access/share links here, then run the matching Refresh button to sync DataForge.
            </p>
          </div>
            ...
            <table className="min-w-full text-xs">
              <thead className="bg-muted/40 text-xs uppercase tracking-wide">
                <tr>
                  ...
                  <th className="text-left px-3 py-2">Usage & Refresh</th>
                  ...
                  <tr key={src.key} className="odd:bg-white even:bg-muted/20 align-top">
                    ...
                    <td className="px-3 py-2">
                      <div className="flex flex-wrap gap-2">
                        ...
                        <Button size="sm" variant="ghost" onClick={() => copySheetId(src.sheetId)}>
                          Copy ID
                        </Button>
                      </div>
                    </td>
      ...
      {activePanel === 'backup' && (
        <div className="space-y-4">
          <div className="rounded-xl border bg-white p-4">
            <div className="flex items-center justify-between mb-2">
              <div>
                <h3 className="font-medium">Backup current data</h3>
                <p className="text-xs text-muted-foreground">Download the canonical dataset and metrics files before making major edits.</p>
              </div>
            </div>
            <div className="flex flex-wrap gap-2">
              <Button size="sm" onClick={downloadCanonical}>Download lg-audits.json</Button>
              <Button size="sm" variant="outline" onClick={downloadAuditConfigFile}>Download audits.config.json</Button>
              <Button size="sm" variant="outline" onClick={downloadUnmatched} disabled={!unmatched.length}>Download latest unmatched list</Button>
            </div>
          </div>
          <div className="rounded-xl border bg-white p-4 space-y-3">
            <div>
              <h3 className="font-medium">Upload & replace lg-audits.json</h3>
              <p className="text-xs text-muted-foreground">Select a validated JSON file to load it into the dashboard and push it to the server. This overwrites the existing lg-audits.json served by Apache.</p>
            </div>
            <div className="flex flex-wrap items-center gap-2">
              <Button size="sm" onClick={() => uploadCanonRef.current?.click()} disabled={uploadingCanon}>
                {uploadingCanon ? 'Uploading…' : 'Select JSON file'}
              </Button>
              {uploadCanonLog && <span className="text-xs text-muted-foreground">{uploadCanonLog}</span>}
            </div>
            <input
              ref={uploadCanonRef}
              type="file"
              ...
              onChange={(e) => {
                ...
                if (file) {
                  if (!window.confirm(`Replace lg-audits.json with ${file.name}? This will overwrite the current data on the server.`)) {
                    e.target.value = ''
                    return
                  }
                  replaceCanonFromFile(file)
                }
              }}
            />
          </div>
