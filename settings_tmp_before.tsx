...
export function SettingsView(){
  ...
  async function onImportFileChange(e: React.ChangeEvent<HTMLInputElement>){
    ...
    try {
      ...
      const miss: Array<{ province: string; lgu: string; key: string; suggest?: string }> = []
      rows.forEach((r: any) => {
        total++
        ...
        if (!rec){ miss.push({ province: prov, lgu, key, suggest: (typeof suggestFor==='function'? suggestFor(key) : undefined) }); return }
